...
def normal_gain(level: int) -> int:
    ...
    if level < 30:
        ...
        weights = [4, 6, 10, 15, 10, 8, 6, 4, 2, 1]
    else:
        weights = [4, 6, 10, 10, 10, 6, 6, 2, 2, 1]
...
class reinforce(commands.Cog):
    ...
    async def ensure_rate_bonus_table(self, cur):
        await cur.execute(
    ...
    async def item_rate_bonus(self, cur, user_id, item_name):
        await self.ensure_rate_bonus_table(cur)
        await cur.execute(
            'SELECT bonus FROM reinforce_rate_bonus WHERE user_id = %s AND item_name = %s',
            (user_id, item_name),
        )
        row = await cur.fetchone()
        return 0 if row is None else int(row['bonus'])

    async def delete_item_rate_bonus(self, cur, user_id, item_name):
        await self.ensure_rate_bonus_table(cur)
        await cur.execute(
            'DELETE FROM reinforce_rate_bonus WHERE user_id = %s AND item_name = %s',
            (user_id, item_name),
        )

    async def rename_item_rate_bonus(self, cur, user_id, old_name, new_name):
        await self.ensure_rate_bonus_table(cur)
        await cur.execute(
            'UPDATE reinforce_rate_bonus SET item_name = %s WHERE user_id = %s AND item_name = %s',
    ...
    async def ask_normal_protection(self, ctx, cur, user_id, weapon, level, amount):
        ...
        if await self.ask_and_consume_item(
            ctx,
            cur,
            user_id,
            item_key='normal_protect',
            item_name='일반강화보호권',
            title='🛡️ 하락 방지 사용',
    ...
    async def send_server_rank(self, ctx):
        ...
        text = '\n\n'.join(
            f'{idx + 1}위 | {name}\n{item_label(item, level)}'
            for idx, (name, item, level) in enumerate(ranking)
        )
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                if await cur.execute('SELECT * FROM reinforce WHERE id = %s and name = %s', (user, weapon)) == 0:
                    ...
                    await cur.execute(
                        'INSERT INTO reinforce VALUES (%s, %s, %s, %s)',
                        (uuid.uuid4().hex, weapon, user, 0),
                    )
                    ...
                    confirmed, _ = await ask_confirm(
                        ...
                        embed=get_embed(
                            ...
                            (
                                f'{current_label}\n\n'
                                f'{stars}성 → {stars + 1}성\n'
                                f'성공 {success}% / 실패 {fail}%{destroy_text}{booster_text}\n\n'
                                f'도전하시겠습니까?'
                            ),
                        ),
                        timeout=30,
                    ...
                    if roll <= success:
                        ...
                        if protect:
                            ...
                            await ctx.send(
                                embed=get_embed(
                                    '💣 스타강화 파괴',
                                    f'{weapon}이 파괴되어 0성 90레벨로 돌아갔습니다.\n{item_label(weapon, new_level)}',
                                    0xFF0000,
                                )
                            )
                ...
                permanent_bonus = await self.item_rate_bonus(cur, user, weapon)
                rate = min(95, normal_success_rate(level) + permanent_bonus)
                bonus_text = f'\n영구 보너스: +{permanent_bonus}%' if permanent_bonus else ''
    ...
    async def _rf_rename(self, ctx, old_name: str, *, new_name: str):
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                ...
                await cur.execute(
                    'UPDATE reinforce SET name = %s WHERE id = %s and name = %s',
                    (new_name, ctx.author.id, old_name),
                )
                await self.rename_item_rate_bonus(cur, ctx.author.id, old_name, new_name)
    ...
    async def _rf_erase(self, ctx, *, arg):
        async with self.pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                ...
                await self.delete_item_rate_bonus(cur, ctx.author.id, arg)
