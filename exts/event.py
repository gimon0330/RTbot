...
async def safe_send_channel(client, channel_id: int, *args, **kwargs):
...
class events(commands.Cog):
    ...
    async def on_error(self, event, *args, **kwargs):
        ...
        await safe_send_channel(self.client, ERROR_LOG_CHANNEL_ID, embed=get_embed("error", errstr))
    ...
    async def on_command_error(self, ctx: commands.Context, error: Exception):
        ...
        if commands.errors.MissingRequiredArgument in allerrs:
            ...
            await ctx.send(embed=get_embed(
                '<a:no:698461934613168199> | 멤버 권한 부족!',
                f'{ctx.author.mention}, 이 명령어를 사용하려면 다음과 같은 길드 권한이 필요합니다!\n`' + '`, `'.join(perms) + '`',
                0xFF0000,
            ))
            return
...
""",
        ...
        if ctx.author.id == OWNER_ID:
            ...
            await safe_send_channel(
                ...
                embed=get_embed('<a:no:698461934613168199> | **ERROR**!', f'Id : {ctx.author.id}\nContent : {ctx.message.content}```python\n{error_text}```', 0xFF0000),
            )
